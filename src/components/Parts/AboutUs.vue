<template>
    <div>
        <h2>Our Story</h2>

        <div>We are a team of five people, currently studying at Barcelona Technology School.
            We almost finished our master’s degree in Digital Solution Development here in Spain.
            We moved here all from all around the world.
        </div>
        <hr>
        <div class="intro"><span>Floris</span>  : is 25 years old and from the Netherlands. He studied marketing in Groningen
            and did his internship in Amsterdam at Ahold. When he was working, he felt that
            he wasn’t finished and moved to Barcelona. Currently, he is a digital marketing
            intern at Trybe app. He specialties are marketing, sales and his social skills.
        </div>
        <hr>
        <div class="intro"><span>Gustavo</span>  : is 32 years old and from Brazil. He is a Digital Marketing
            Specialist with more than 8 years of experience, working with companies
            as Honda Motors, Nextel, A&E Channel and The History Channel. Currently
            he is a digital marketing intern at Learnlight.
        </div>
        <hr>
        <div><span>Arin</span>: </div>
        <hr>
        <div class="intro"><span>Bin</span>  : is 25 years old from Russia. He has finance background but in the
            same time has a huge interest in web development and data science.
            Going to Barcelona to make the first step to the technical world.
            Currently working in Argubi.tech.
        </div>
    </div>

</template>

<script>
    export default {

    }

</script>

<style scoped>
    span {
        font: 200 50px/1.3 'Arizonia', Helvetica, sans-serif;
        color: #2b2b2b;
        text-shadow: 4px 4px 0px rgba(0,0,0,0.1);
    }

</style>